...
The fixed envelope header is 22 bytes. `client_id` is limited to 64 bytes and
the 16-bit `payload_length` field permits at most 65,535 payload bytes.
...
| `5` | `KEEPALIVE` | Client -> Server | Prove established-session liveness |
| `6` | `KEEPALIVE_ACK` | Server -> Client | Acknowledge keepalive |
...
- `key_exchange_payload` is verified by the future crypto/key-exchange module.
...
- `session_id`
- `timestamp_ms` for `KEEPALIVE`, or `original_timestamp_ms` for
  `KEEPALIVE_ACK`
- `authenticator`
...
- If the datagram source differs from the stored `peer_address`, the server may
  update `peer_address` only after authentication and replay checks pass. An
  ACK refreshes liveness only and never changes `peer_address`.
...
- Each pending message is transmitted at most 4 times total: the initial send
  followed by retries at 1s, 3s, and 7s from handshake start. Retry exhaustion
  immediately fails and cleans up the in-progress session.
